...
build_slice() {
    ...
    # face_detect / matting：引擎 API 在核心库，mobile 照常编译（STATIC；引擎
    # 产物缺失时任务注册但运行时报可读错误——stub 语义由核心库承载）
    submodules+=(face_detect matting)
...
merge_static_libs() {
    ...
    for sub in ${OPENCV_SUBMODULES} gpu_image_processing face_detect matting; do
