...
- Test models **and images** are downloaded (not committed) via `scripts/download_mediapipe_models.sh` into `submodules/mediapipe/mediapipe_vision/tests/models/`; each `test_mediapipe_*` SKIPs if its model/image is absent. Tests link the plugin directly, build a `TaskContext` with `inputs_by_port["image"]`, and run CPU + GPU (GPU soft-SKIPs if Metal delegate init fails — common on macOS for landmarker models). The Face/Pose/ObjectDetector tests use a royalty-free portrait (`portrait.jpg`, Unsplash); the Hand test uses a dedicated hand close-up (`hand_image.jpg`, CC BY-SA 4.0 by Adams890 via Wikimedia Commons — attribution kept in the download script) since hand landmarks aren't produced from generic portraits. Real images reliably trigger all four tasks; landmark range checks use a wide `[-5,5]` band because MediaPipe extrapolates out-of-frame body parts well beyond `[0,1]`.
