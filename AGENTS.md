...
- The subnode CMakeLists links `libvision.dylib` when present in `build/mediapipe/install/lib`; otherwise it compiles a stub that makes no C API calls. **ObjectDetector is now fully wired** (IMAGE mode): `run_image()` does Image→MpImage conversion then dispatches to a virtual `run_inference()` hook; `ObjectDetectorTask::on_init()` reads `model_path`/`delegate`/`max_results`/`score_threshold` from config and calls `MpObjectDetectorCreate` (handle reused across `execute()`, closed in the destructor); `run_inference()` calls `MpObjectDetectorDetectImage`, maps `DetectionResult`→`VisionResult.detections` (bbox normalized to [0,1]), and frees via `MpObjectDetectorCloseResult`. Face/Hand/Pose still use the base no-op `run_inference` (not yet wired).
- Test models are downloaded (not committed) via `scripts/download_mediapipe_models.sh` into `submodules/mediapipe/mediapipe_vision/tests/models/`; `test_mediapipe_object_detector` SKIPs if the model is absent. The test links the plugin directly and builds a `TaskContext` with `inputs_by_port["image"]`.
- `libvision.dylib` ships with a **bare install name** (`libvision.dylib`, not `@rpath/...`), so LC_RPATH doesn't apply and `DYLD_LIBRARY_PATH` is stripped under SIP. The test symlinks the prebuilt next to the binary (`add_custom_command POST_BUILD`) and sets `WORKING_DIRECTORY` so dyld's CWD lookup finds it. The real app/plugin loader must do likewise (copy/symlink or fix the install name).
