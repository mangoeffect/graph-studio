...
Median filter (`cv::medianBlur`). Far better than linear filters on impulse noise (salt & pepper) and it doesn't smear edges like the mean does, at a higher compute cost.

**Module**：OpenCV filtering · **Task type**：`opencv_median_blur_filter`
...
```json
{
  ...
  "tasks": [
    ...
    {
      "id": "node",
      "type": "opencv_median_blur_filter",
      "params": {
        "kernel_size": 5
      }
    },
