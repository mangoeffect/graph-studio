...
Saturating subtraction `saturate(in - in2)`. Combined with `render_dilate_dir`/`render_erode_dir` outputs it forms morphological gradients (dilate − original, original − erode), or frame differencing.

**Module**：GPU render · **Task type**：`render_subtract`
